...
internal fun buildSettingsSyncEvents(
    ...
    return buildSettingsSyncEvents(previous.syncedCollections(), next.syncedCollections())
}

internal fun buildInitialSettingsSyncEvents(
    settings: AppSettings,
): List<PallaSyncPendingEvent> {
    return buildInitialSettingsSyncEvents(settings.syncedCollections())
}

internal fun buildInitialSettingsSyncEvents(
    collections: SyncedCollectionsSnapshot,
): List<PallaSyncPendingEvent> {
    return buildSettingsSyncEvents(SyncedCollectionsSnapshot(), collections)
}

internal fun buildSettingsSyncEvents(
    previous: SyncedCollectionsSnapshot,
    next: SyncedCollectionsSnapshot,
): List<PallaSyncPendingEvent> = buildList {
    addStringSetChanges(
    ...
    if (previous.searchHistory != next.searchHistory) {
        ...
        previous.searchHistory.distinct()
    ...
    if (previous.seenFeedIllusts != next.seenFeedIllusts) {
        ...
        previous.seenFeedIllusts.distinct()
    ...
    if (previous.viewHistory != next.viewHistory) {
        ...
        previous.viewHistory.distinctBy(Illust::id)
...
private fun <T, K> orderedMovedPrefix(
    ...
    for (prefixSize in 0..nextDistinct.size) {
        ...
        val untouchedPrevious = previousDistinct.filter { item ->
...
internal suspend fun PalleriaSyncManager.enqueueInitialSettings(
    settings: AppSettings,
) {
...
private fun seenIllustEvent(id: Long, operation: String): PallaSyncPendingEvent {
    return PallaSyncPendingEvent(
...
private fun muteEvent(kind: String, value: String, operation: String): PallaSyncPendingEvent {
    return PallaSyncPendingEvent(
...
private fun viewedIllustEvent(illust: Illust, operation: String): PallaSyncPendingEvent {
    return PallaSyncPendingEvent(
        schema = VIEW_HISTORY_SCHEMA_V2,
